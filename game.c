...
//draw Tinker Bell
void drawTink(Tink *tptr) {
  drawImage3(tptr->posY, tptr->posX, TINK_WIDTH, TINK_HEIGHT, tink);
}

//draw flower
void drawFlower(Flower *fptr) {
  drawImage3(fptr->posY, fptr->posX, FLOWER_WIDTH, FLOWER_HEIGHT, flower);
}

//draw beast
void drawBeast(Beast *bptr) {
  drawImage3(bptr->posY, bptr->posX, BEAST_WIDTH, BEAST_HEIGHT, beast);
}

//check for collision
int collide(Flower *fptr, Beast *bptr) {
  int flower_y = fptr->posY;
  int flower_x = fptr->posX;
  int beast_y = bptr->posY;
  int beast_x = bptr->posX;
  if (flower_y < (beast_y + BEAST_HEIGHT) && (flower_y + FLOWER_HEIGHT) > beast_y) {
    if (flower_x < (beast_x + BEAST_WIDTH) && (flower_x + FLOWER_WIDTH) > beast_x) {
      return 1;
    }
  }
  return 0;
...
const unsigned short sinArray[256] = {
